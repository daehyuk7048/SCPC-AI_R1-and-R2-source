...
SENSITIVE_CONTAINS = {"rrn", "raw_quote", "location", "doctor_note",
                      "numeric_value", "raw_memory", "raw_sensitive"}
...
class FinalHarness:
    ...
    def answer_task(self, task: dict[str, Any], session: dict[str, Any]) -> dict[str, Any]:
        ...
        target = self.infer_target(task, control)
    ...
    # ---- 3) target ----
    def infer_target(self, task: dict[str, Any], control: str) -> str:
        ...
        b = (task.get("prompt", "") or "") + " " + \
            " ".join(h.get("summary", "") for h in task.get("visible_history", []) or [])
    ...
    def _profile_channel(self, blob, prof):
        ...
        # reuse a prior success / apply a stored enterprise policy -> approval channel
        if _has(blob, "성공한 방식", "성공한 작업", "지난번 성공", "회사 기본 규정", "저장된 회사"):
            return prof.get("approval_channel")
        # personal / health context (birthday, gift, checkup, tone) -> preferred channel
    ...
    def build_content_scope(self, task, focal_id, control, target) -> dict[str, Any]:
        ...
        mode = self._mode_for(control, target)
        return {
            "mode": mode,
            "allowed_fields": self._allowed_for(mode),
            "excluded_fields": self._excluded_for(mode, cts),
    ...
    def _mode_for(self, control, target):
        if control == "hold":
            return "none"
        if control == "amend":
            return "redacted"
        if control == "proceed":
            return "status_only" if target in LOCAL_TARGETS else "raw"
        if control == "ask":
            return "summary"
        return "status_only"
    ...
    def _excluded_for(self, mode, cts):
        if mode in ("none", "raw"):
            return []
        if mode == "status_only":
    ...
    def build_policy(self, task, focal_id, control, target) -> dict[str, Any]:
        ...
        if (_first_rec(task, "route_candidate_snapshot") == "single_internal_candidate"
                and _first_rec(task, "dispatch_authority_check") == "internal_binding_confirmed"):
    ...
    def build_plan_events(self, task, focal_id, control, target, scope) -> list[dict[str, Any]]:
        ...
        if control == "ask":
            # purpose tracks the structural precondition_changed signal (dev: 26/26)
            prec = (_first_rec(task, "route_candidate_snapshot") == "single_internal_candidate"
                    and _first_rec(task, "dispatch_authority_check") == "internal_binding_confirmed")
            reason = "clarify_precondition" if prec else "route_resolution_required"
            return [
    ...
    def _user_response(self, control, target, scope):
        ...
        if control == "ask":
            return "최종 수신처와 공유 범위가 아직 확정되지 않아 먼저 확인이 필요합니다."
        if control == "amend":
            return f"민감한 세부값을 제외한 요약만 {target}(으)로 공유하겠습니다."
