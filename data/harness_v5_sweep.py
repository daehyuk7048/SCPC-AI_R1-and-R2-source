...
class FinalHarness:
    ...
    def _approved_external_channel(self, task):
        ...
        legal_review) from the dispatch-authority record rather than a per-control guess."""
        dac = _first_rec(task, "dispatch_authority_check")
        sig = str(_first_rec(task, "guardrail_ladder_signal") or "")
        if dac == "user_binding_pending":
            return "project_room"
        if dac == "authority_incomplete":
    ...
    def build_content_scope(self, task, focal_id, control, target) -> dict[str, Any]:
        ...
        mode = self._mode_for(task, control, target)
        return {
            "mode": mode,
            "allowed_fields": self._allowed_for(mode),
            "excluded_fields": self._excluded_for(task, mode, cts),
    ...
    def _mode_for(self, task, control, target):
        if control == "hold":
            return "none"
        if control == "amend":
            return "redacted"
        if control == "proceed":
            ...
            if _first_rec(task, "share_boundary_update") == "local_update_boundary":
                return "status_only"
            if target == "legal_review":       # a formal review channel gets a summary, not a raw dump
                return "summary"
            return "raw"
        if control == "ask":
            if _first_rec(task, "session_share_policy") == "normal":
                if _first_rec(task, "target_changed_after_turn") == "prior_success_route_superseded":
                    return "none"
                return "summary"
            # strict policy: formal compliance/review channels require field redaction
            if target in ("audit_vendor", "legal_review"):
                return "redacted"
            return "summary"
        return "status_only"
    ...
    def _excluded_for(self, task, mode, cts):
        if mode in ("none", "raw"):
            return []
        if mode == "status_only":
