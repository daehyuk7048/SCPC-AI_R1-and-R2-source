...
class FinalHarness:
    ...
    def _mode_for(self, task, control, target, focal_id=None):
        ...
        if control == "ask":
            ...
            if fo is not None and _is_formal_channel((fo.get("attrs") or {}).get("recipient") or ""):
                return "redacted"
            # bouncing back to the user under an explicit external-redaction boundary ->
            # show a redacted preview rather than a plain summary
            if target == "user" and _first_rec(task, "share_boundary_update") == "redacted_external_boundary":
                return "redacted"
            return "summary"
        return "status_only"
