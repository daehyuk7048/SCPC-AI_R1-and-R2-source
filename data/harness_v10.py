...
class FinalHarness:
    ...
    def decide_control(self, task: dict[str, Any]) -> str:
        ...
        hist = " ".join(h.get("summary", "") for h in task.get("visible_history", []) or [])
    ...
    # ---- 3) target ----
    def _approved_channel_from_recipient(self, task, focal_id, control):
        """The focal object's own recipient IS the approved channel for the
        approved_channel_or_visible_recipient ambiguity — dev: a NEUTRAL approved-channel
        task's target equals the recipient (project_room->project_room, legal_review->legal_review,
        …), with a privacy recipient instead addressed to the user under ask/hold. An explicit
        ask/hold user-correction is handled upstream. Empty recipient -> dispatch-authority resolver."""
        fo = _focal_obj(task, focal_id)
        rec = str((fo.get("attrs") or {}).get("recipient") or "") if fo else ""
        if "privacy" in rec.lower():
            return "user" if control in ("ask", "hold") else None
        return rec or None
    ...
    def infer_target(self, task: dict[str, Any], control: str, focal_id: str = "") -> str:
        ...
        if control == "amend":
            ...
            if amb == "approved_channel_or_visible_recipient":
                ch = self._approved_channel_from_recipient(task, focal_id, "amend")
                if ch:
                    return ch
        ...
        if control == "ask":
            ...
            if amb == "approved_channel_or_visible_recipient":
                ch = self._approved_channel_from_recipient(task, focal_id, "ask")
                if ch:
                    return ch
            return "user"
        ...
        if control == "hold":
            ...
            if "조명" in b and has_recall:
                return "living_room"
            if amb == "approved_channel_or_visible_recipient":
                ch = self._approved_channel_from_recipient(task, focal_id, "hold")
                if ch:
                    return ch
                ac = self._approved_external_channel(task)
                if ac:
