    ...
    def infer_target(self, task: dict[str, Any], control: str, focal_id: str = "") -> str:
        ...
        if control == "proceed":
            ...
            if amb == "approved_channel_or_visible_recipient":
                ...
                if ch:
                    return ch
            # a recalled health/checkup share reuses the person's stored health channel
            # (the same profile routing the amend/ask branches use, applied to proceed)
            if has_recall and prof and _has(b, "검진", "점검", "복약", "병원", "건강"):
