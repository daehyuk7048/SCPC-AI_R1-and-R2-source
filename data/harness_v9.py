...
class FinalHarness:
    ...
    def _approved_channel_from_recipient(self, task, focal_id, control):
        ...
        approved_channel_or_visible_recipient ambiguity. Dev-validated cells only:
        legal/audit recipient -> that channel (amend/ask/hold); privacy recipient -> user
        (ask/hold only). project_room / other recipients fall to the dispatch-authority resolver."""
        fo = _focal_obj(task, focal_id)
        rec = str((fo.get("attrs") or {}).get("recipient") or "") if fo else ""
        r = rec.lower()
        if "legal" in r or "audit" in r:
            return rec
        if "privacy" in r and control in ("ask", "hold"):
            return "user"
        return None
