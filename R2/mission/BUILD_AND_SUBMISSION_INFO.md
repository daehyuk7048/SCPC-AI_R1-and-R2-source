...
├── gradlew(.bat) · gradle/wrapper/            # Gradle 8.9 wrapper 동봉(설치 불필요 — 배포판은 첫 빌드 시 자동 다운로드, §2)
...
의존성은 Kit AAR + Android 플랫폼 API가 전부이며 런타임 외부 라이브러리가 없다(테스트 전용: junit 4.13.2, org.json 20240303).

**첫 빌드에는 인터넷 연결이 필요하다.** 동봉된 wrapper는 런처 JAR·설정 파일뿐이므로, 첫 실행 시 Gradle 8.9 배포판(`gradle-wrapper.properties`의 services.gradle.org distributionUrl), AGP 8.7.3·Kotlin 2.0.21 플러그인(`settings.gradle.kts`의 google()/mavenCentral()/gradlePluginPortal), 테스트 의존성(google()/mavenCentral())이 자동 다운로드된다. 캐시가 채워진 뒤에는 오프라인 재빌드가 가능하다.

**주의:** AGP는 비ASCII 문자가 포함된 경로에서 빌드를 거부할 수 있으므로 소스는 ASCII 경로에 압축 해제한다(참가자 Windows 환경 실측). SDK 위치는 `ANDROID_HOME` 환경변수 또는 소스 루트의 `local.properties`(`sdk.dir=...`)로 지정한다.
...
# 산출물: app/build/outputs/apk/release/ 아래
#   keystore 구성 시(§4): app-release.apk / 미구성 시: app-release-unsigned.apk
...
`app/build.gradle.kts`의 서명 방식(파일 실확인): 소스 루트 **밖** 상대경로 `../keystore/keystore.properties`를 `java.util.Properties`로 읽어, 파일이 존재하면 그 안의 `storeFile`·`storePassword`·`keyAlias`·`keyPassword` 4개 값으로 release signingConfig를 구성한다. 파일이 없으면 서명 설정 블록 자체를 만들지 않으므로 **secret 없이도 빌드는 그대로 성공**하고 unsigned APK(`app-release-unsigned.apk`)가 나온다.
...
core는 실시계·난수·모델을 쓰지 않는 결정론 구현이고 release 빌드는 minify 없이 고정 입력만 사용한다. 재현성 주장은 실측 범위로 한정한다: **동일 환경(참가자 Windows, §2와 동일한 도구 버전)에서 clean 후 재빌드를 반복했을 때 산출 APK의 SHA-256이 동일함을 확인했다.** OS·파일시스템·도구 세부 버전 등 환경이 다르거나 서명 키 종류가 다르면 바이트 단위 결과는 달라질 수 있으므로, 환경 간 byte 동일성은 주장하지 않는다.

이에 따라 심사측 검증의 1차 수단은 해시 일치가 아니라 **unsigned 산출물끼리의 비교 또는 `apkanalyzer`를 이용한 APK 내용 대조**다. APK 전체 해시에는 서명 블록이 포함되므로, 심사측이 자체 keystore로 서명하면 제출 APK와 전체 해시는 당연히 달라진다. 해시 확인 명령:

```text
gradlew clean :app:assembleRelease
# 대상 파일명: keystore 구성 빌드는 app-release.apk, 미구성 빌드(§4)는 app-release-unsigned.apk
# Windows PowerShell
...
SHA-256·인증서 지문 값 자체는 Kit 규정에 따라 문서에 수기로 기재하지 않는다 — 제출 완성 도구가 실파일에서 계산한다.
